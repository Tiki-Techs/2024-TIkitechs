...
public class TwoNote extends Command {
  ...
  public TwoNote(SwerveSubsystem swerve, Shooter shooter, Intake intake, Hood hood, boolean fieldRelative) {
  ...
  public void execute() {
    ...
    RobotContainer.s_Shooter.quickReverse();
    while (m_timer.get() < 10) {

    }
    while (m_timer.get() < 13) {
      RobotContainer.s_Shooter.m_pidController.setReference(RobotContainer.s_Shooter.getAutomaticState().speed,
          ControlType.kVelocity);
      var set = Math.max(HoodConstants.lowerLimit, Math.min(RobotContainer.s_Shooter.getAutomaticState().angle, HoodConstants.upperLimit));
      RobotContainer.positioner.m_Leader.set(RobotContainer.positioner.hoodPID
