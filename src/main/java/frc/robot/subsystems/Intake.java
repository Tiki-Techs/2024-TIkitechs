...
public class Intake extends SubsystemBase {
    ...
    // this method is the default command of this subsystem, so it is always
    // repeating and running
    public void run() {

        // if the A button is pressed run this
        if (RobotContainer.mechXbox.getAButton()
                || (!RobotContainer.overrideNoteSensor && !RobotContainer.s_Shooter.HasNote())) {
            ...
            // the y button is opposite to the A button, so we want it to reverse the intake
            // so we can spit something out if it gets stuck.
            mot.set(-1);
        } else {
    ...
    // sets the intake motor speed to 1 and the index to -1. This is how we pick up
    // notes.
    // When using .set() the motor will stay at that speed until it is told to
    // change
    ...
    public Command StopIntake() {
        return new InstantCommand(() -> {
            ...
            while (m_timer.get() < 0.5) {
                // keeps the intake running for half a second so that we make sure the note gets
                // picked up all the way.
                mot.set(1);
                RobotContainer.s_Shooter.index.set(-1);
            }
            ...
            while (m_timer.get() < 0.7) {
                // runs the index motor backwards for 0.2 seconds so that the note isnt pressed
                // up against the shooter wheels
