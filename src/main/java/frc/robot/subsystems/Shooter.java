...
public class Shooter extends SubsystemBase {
    ...
    public DigitalInput IRSensor = new DigitalInput(Constants.ShooterConstants.IRSensor_ID);
    ...
    public void RunShooter(double speed) {
        ...
        //auto shooting
        //sets the angle of the hood and the speed setpoint equal to the autogenerated setpoints given our distance from the speaker
        if (RobotContainer.mechXbox.getRightBumper()) {

            //we do this in case the limelight is flickery when seeing the april tag, or if we want to rev up to speed before reaching the speaker.
    ...
    public void periodic() {
        if (setpoint != 0) {
            //set the motor to run to reach the setpoint
            m_pidController.setReference(setpoint, ControlType.kVelocity);
        } else {
            //if the setpoint is zero, stop running the motor (Prevents wobble)
            m_leader.set(0);
        }
    ...
    public Command Fire() {
        return new InstantCommand(
                () -> {
                    ...
                    a_timer.reset();
                    a_timer.start();
                    //stay aiming and up to speed for 0.5 seconds wile also running the index to fire the shot.
                    while (a_timer.get() < 0.5) {
                        m_pidController.setReference(5700, ControlType.kVelocity);
                        if (Vision.distance != 0) {
                            var set = Math.max(HoodConstants.lowerLimit,
                                    Math.min(getAutomaticState().angle - Constants.HoodConstants.AutoOffset, HoodConstants.upperLimit));
                            RobotContainer.positioner.m_Leader
                                    .set(RobotContainer.positioner.hoodPID
                                            .calculate(RobotContainer.s_Hood.getRotation(), set));
                        } else {
                            //if we cannot see the april tag dont run the hood that way it doesnt crash into itself.
    ...
    public Command StopShooter() {
        return new InstantCommand(() -> {
            ...
            //stops running the firing motors and for 0.5 seconds move the hood back down to the intake position
            while (a_timer.get() < 0.5) {
